...
**Prerequisites**  
For this chapter we need **Visual Studio 2022** with the **.NET Compiler Platform SDK** installed.

## Open *Visual Studio 2022*

## Create a new *Console App*

Make sure to choose the **non** *.NET Framework* version.
...
With the `CSharpSyntaxTree.ParseText` method you can parse text into a syntax tree.

Use the source code of the `Program.cs` file that was part of the `ConsoleApp1` project.  
For readability, use [raw string literals][RAW_STRING_LITERALS].
...
To compile the source code you to create a `CSharpCompilation` object with a name for the assembly. This name does not really matter for this workshop.  
...
Until now, the text `WriteLine` had only some meaning to you from your experience, but it is just a plain text word in a text file.
...
The `Console.WriteLine` method has eighteen overloads.  
